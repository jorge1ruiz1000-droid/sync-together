...
export function VerifyPage({ tokenOverride }: { tokenOverride?: string } = {}) {
  const navigate = useNavigate();
  const [token, setToken] = useState(tokenOverride ?? "");
  ...
  useEffect(() => {
    ...
    // Support links of the form /verify/<token> by extracting the last path segment.
    try {
  ...
  return (
    <AuthCard
      ...
      subtitle="Paste the token from your verification email or click Verify to continue."
      onSubmit={() => void submit()}
    >
      {/* Token is supplied via the URL and should not be shown to the user. */}
